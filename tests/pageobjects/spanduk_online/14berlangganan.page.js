...
class bannerBerlangganan extends Page {
  ...
  async insertGambarBanner() {
    ...
    const remoteFilePath = path.join(
      __dirname,
      "../../files-to-upload/spanduk_online/2.png"
    );
  ...
  async tipeBerlangganan() {
    await this.selectBox[2].scrollIntoView();
    await this.selectBox[2].click();
    // await expect(this.listBoxStatusBerlangganan[0]).toBeClickable();
    // await this.listBoxStatusBerlangganan[0].click();
    await this.optionTipeBerlangganan[0].click();
  }
...
export default new bannerBerlangganan();
