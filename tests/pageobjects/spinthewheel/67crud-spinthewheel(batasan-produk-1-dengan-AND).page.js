...
class stwBatasanProduk1AND extends Page {
  get titlePage() {
  ...
  async batasanBerdasarkanProduk1(batasan_berdasarkan_produk) {
    await this.ContainerPB[0].scrollIntoView();
  ...
  async batasanBerdasarkanProduk2(batasan_berdasarkan_produk) {
    await this.ContainerPB[0].scrollIntoView();
    await this.inputSearchBatasanBerdasarkanProduk.setValue(
      batasan_berdasarkan_produk
    );
    await browser.pause(4000);
    await this.optionBatasanBerdasarkanProduk[1].click();
  }

  async nilaiTransaksiMinimal1(nilai_transaksi_minimal) {
    await this.inputNilaiTransaksiMinimal[0].setValue(nilai_transaksi_minimal);
  }

  async nilaiTransaksiMinimal2(nilai_transaksi_minimal) {
    await this.inputNilaiTransaksiMinimal[1].setValue(nilai_transaksi_minimal);
  }
  ...
  async InputXP1(xp1) {
    await this.inputCoin[2].scrollIntoView();
    await this.inputCoin[2].setValue(xp1);
  }

  async InputXP2(xp2) {
    await this.inputCoin[3].setValue(xp2);
  }

  async InputXP3(xp3) {
    await this.inputCoin[4].setValue(xp3);
  }
...
export default new stwBatasanProduk1AND();
